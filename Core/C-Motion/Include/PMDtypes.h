...
// set the calling convention to __stdcall when building a DLL
# define PMD_CCONV __stdcall
# ifdef PMD_EXPORTS
// For the library build
// __declspec(dllexport) is required if there is no .def file
...
typedef enum {
        PMDProductInfoVersion                                   = 1, // beta  |  custom   |  major  | minor
        PMDProductInfoProductClass                              = 2, // class | motortype | numaxes |  0 
...
typedef enum {
        ...
        PMDMotorTypeClosedLoopStepping                          = 1,
...
typedef enum {
        ...
        PMDTraceVariableMotionICTime                            = 8,
